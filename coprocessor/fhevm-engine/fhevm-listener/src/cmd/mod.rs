...
pub struct Args {
    ...
    #[arg(long, default_value = "100", help = "Catchup paging size")]
...
impl InfiniteLogIter {
    ...
    async fn consume_catchup_blocks(&mut self) {
        ...
        if Some(paging_to_block) == to_block {
            info!("No Future Catchup");
            self.catchup_blocks = None;
        } else if to_block.is_some() {
            self.catchup_blocks = Some((paging_to_block + 1, to_block));
            info!(
                from_block = paging_to_block + 1,
                to_block = to_block,
                "Future Catchup"
            );
        }
